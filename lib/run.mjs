...
async function getClockfaceView(state, runtime) {
  ...
  return {
    ...
    intervalLabel: getIntervalLabel(instance),
...
function getFrameDelayMs(instance) {
  ...
  return Math.max(updateIntervalMs, getSimulatedPacingMs(instance.resolution));
}

function getSimulatedPacingMs(resolution) {
  return SIMULATED_PACING_MS.get(resolution) ?? 0;
}

function getIntervalLabel(instance) {
  const updateIntervalMs = Number(instance.updateIntervalMs ?? 0);
  ...
  const pacingMs = getSimulatedPacingMs(instance.resolution);

  if (pacingMs > updateIntervalMs) {
    return `${updateIntervalMs}ms (capped to ${pacingMs}ms in ${instance.resolution}x${instance.resolution} mode)`;
  }

  return `${updateIntervalMs}ms`;
}
    ...
    <style>
      ...
      * {
      ...
      body {
        ...
        min-height: 100vh;
        background:
          radial-gradient(circle at 18% 12%, rgba(114, 212, 255, 0.16), transparent 28%),
          radial-gradient(circle at 88% 18%, rgba(255, 214, 80, 0.12), transparent 26%),
          linear-gradient(135deg, #0e1117 0%, #111722 52%, #0d1016 100%);
        display: grid;
      ...
      .meta, .error {
        ...
        border-radius: 8px;
        padding: 14px;
        background: linear-gradient(180deg, rgba(29, 36, 49, 0.96), rgba(21, 26, 36, 0.96));
      ...
      .field {
        display: grid;
        gap: 7px;
        border: 1px solid rgba(48, 57, 74, 0.76);
        border-radius: 8px;
        padding: 10px;
        background: rgba(23, 28, 38, 0.76);
      }
      ...
      input:focus, select:focus, button:focus {
        outline: 2px solid rgba(114, 212, 255, 0.42);
        outline-offset: 1px;
      }

      button {
        cursor: pointer;
        background: linear-gradient(180deg, #31415c, #263246);
        font-weight: 650;
      }
  ...
  <body>
    <main>
      <header>
        <div>
          ...
          <p class="subtitle">Live Clockface preview</p>
        </div>
        <div class="pill">PixooPal Preview</div>
      </header>
    ...
    <script>
      ...
      async function loadClockface() {
        ...
        meta.innerHTML =
          '<div><strong>Author</strong><span>' + escapeHtml(view.author || 'Unknown') + '</span></div>' +
          '<div><strong>Resolution</strong><span>' + view.resolution + 'x' + view.resolution + '</span></div>' +
          '<div><strong>Interval</strong><span>' + escapeHtml(view.intervalLabel) + '</span></div>' +
          '<div><strong>Build</strong><span>' + escapeHtml(view.buildPath) + '</span></div>';
